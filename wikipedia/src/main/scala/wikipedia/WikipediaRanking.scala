...
object WikipediaRanking extends WikipediaRankingInterface:
  ...
  val conf: SparkConf =
    new SparkConf().setMaster("local").setAppName("my-wikipedia-app")
  ...
  val wikiRdd: RDD[WikipediaArticle] =
    sc.parallelize(WikipediaData.lines.map(line => WikipediaData.parse(line)))
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd
      ...
      .aggregate(0)((acc, _) => acc + 1, (accA, accB) => accA + accB)
  }
  ...
  def makeIndex(
    ...
    val rddPairs = rdd.flatMap(article =>
      langs
  ...
  def rankLangsReduceByKey(
    ...
    val counts = rddPairs.mapValues(_ => 1).reduceByKey(_ + _)
