...
object TimeUsage extends TimeUsageInterface:
  ...
  def classifiedColumns(
    ...
    val primary = columnNames
      .filter(s =>
        s.startsWith("t01") | s.startsWith("t03") | s.startsWith("t11") | s
          .startsWith("t1801") | s.startsWith("t1803")
      )
      .map(x => col(x))

    val working = columnNames
      .filter(s => s.startsWith("t05") | s.startsWith("t1805"))
      .map(x => col(x))

    val other = columnNames
      .filterNot(s =>
        s.startsWith("t01") | s.startsWith("t03") | s.startsWith("t11") | s
          .startsWith("t1801") | s.startsWith("t1803") | s.startsWith("t05") | s
          .startsWith("t1805")
      )
      .map(x => col(x))
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = primaryNeedsColumns.reduce((a, b) => a + b) / lit(60.0)
    val workProjection: Column = workColumns.reduce((a, b) => a + b) / lit(60.0)
    val otherProjection: Column = otherColumns.reduce((a, b) => a + b) / lit(60.0)
    df
      .select(
