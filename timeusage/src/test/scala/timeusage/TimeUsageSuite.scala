...
class TimeUsageSuite extends munit.FunSuite {
  ...
  val spark: SparkSession = SparkSession
   ...
   test("Column classification should work as expected") {
    val colNames: List[String] = List(
      "tucaseid",
      "gemetsta",
      "gtmetsta",
      "peeduca",
    ...
    val groups = classifiedColumns(colNames)
    val (primary, work, other) = groups

    assert(other.filter(c => primary.contains(c)).isEmpty)
    assert(other.filter(c => work.contains(c)).isEmpty)
  }
  ...
  test("timeUsageSummary computes correct projections and filtering") {
    ...
    val actual = rows.map { row =>
      (
        ...
        row.getDouble(3), // primaryNeeds result
        row.getDouble(4), // work result
        row.getDouble(5)  // other result
      )
    }.toSet
